...
- `src/components/CellViewport.tsx` — Individual cell wrapper mounting specific canvas renderers (Orbital Clock, Piano Triangles, Note Stream, Overtone Waves, Staff Stream) with local HUD controls, conveyor mode toggles, fixed queue length sliders (2..24), staff-specific configuration, and granular visual effect toggles (note entrance animation, origin sparks, termination dissipation, voice leading undulation, triangle sparks, triangle shockwaves, triangle lens flares).
...
- `src/components/SettingsDrawer.tsx` — Slide-out settings sidebar providing granular controls for pitch clock, virtual piano keyboard range presets, overtones wave simulation, Staff Stream notation (staff size, dynamic/manual clefs, C-clef toggle, key signature, voice leading lines, note entrance animation, origin sparks, boundary dissipation, voice leading undulation), cosmetics (including reactive note sparks, note activation shockwaves with expansion radius, speed, and dissolve duration sliders, piano triangle sparks and lens flares, halation and starburst decay duration sliders 200ms–3500ms), display effect toggles (including kinetic tonic shift effects), priority slots, themes, audio synthesiser, and real-time focus mode.
- `src/components/InfoModal.tsx` — Comprehensive introduction and theory guide modal dialogue featuring continuous single-piece scrolling, interactive top bookmark navigation, documentation of all 5 cell types (Orbital Clock, Piano Triangles, Note Stream, Overtone Waves, Staff Stream), rich vector diagrams for PPT concepts, Concept 2 on Why Default Do = D (Alphabet, Keyboard & Polar Symmetry), Concept 9 on the Philosophy of PPT Geometric Noteheads, Concept 10 on Staff Stream Notation & Conveyor Architecture, and a "Play Demo" call-to-action button launching the "Boot Up" demo track.
- `src/components/MidiPermissionModal.tsx` — Pre-permission explainer dialogue presenting clean, reassuring, non-technical context on read-only MIDI note listening before browser permission prompts appear, with direct on-screen keyboard fallback.
...
- **Web MIDI Pre-Permission UX & Fallback**: When Web MIDI permissions are in `'prompt'` state, clicking MIDI connection actions opens `MidiPermissionModal` to reassure users in plain language that the app only reads played notes, before the browser prompt is triggered. When permissions are `'denied'`, the toolbar dropdown displays clear step-by-step browser unblock instructions and a direct 1-click fallback to the on-screen virtual keyboard.
