...
- `src/components/InfoModal.tsx` — Comprehensive introduction and theory guide modal dialogue featuring continuous single-piece scrolling, interactive top bookmark navigation, documentation of all 5 cell types (Orbital Clock, Piano Triangles, Note Stream, Overtone Waves, Staff Stream), rich vector diagrams for PPT concepts, Concept 8 on the Philosophy of PPT Geometric Noteheads, Concept 9 on Staff Stream Notation & Conveyor Architecture, and a "Play Demo" call-to-action button launching the "Boot Up" demo track.
