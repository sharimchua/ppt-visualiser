...
- `src/components/SettingsDrawer.tsx` — Slide-out settings sidebar providing granular controls for pitch clock, virtual piano keyboard range presets, overtones wave simulation, Staff Stream notation (staff size, dynamic/manual clefs, C-clef toggle, key signature, voice leading lines, note entrance animation, origin sparks, boundary dissipation, voice leading undulation), cosmetics (including piano triangle sparks and lens flares), display effect toggles (including kinetic tonic shift effects), priority slots, themes, audio synthesiser, and real-time focus mode.
